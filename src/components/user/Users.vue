    ...
    <el-dialog title="修改用户" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="70px">
        ...
        <el-form-item label="手机" prop="mobile">
          <el-input v-model="editForm.mobile" disable></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    addUser() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        if (res.meta.state !== 201) this.$message.error("添加失败!");
    ...
    editUserInfo() {
      this.$refs.editFormRef.validate(async vaild => {
        if (!vaild) this.$message.error("错误!");
        //发起修改数据请求
    ...
    async removeUserbyId(id) {
      //弹框询问用户是否删除
      const removeUser = await this.$confirm(
        "此操作将永久删除该用户, 是否继续?",
        "提示",
        {
      ...
      //取消显示 cancel,确定显示confirm
      //console.log(removeUser);
      if(removeUser!=='confirm')return this.$message.info('已取消删除')
      // console.log('确认了删除');
